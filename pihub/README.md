...
| `NAVIDROME_DATA_PATH` / `JELLYFIN_CONFIG_PATH` / `PITUNE_DATA_PATH` | `./navidrome/data` / `./jellyfin/config` / `./pitune/backend/data` | Per-service config, deliberately off `MEDIA_ROOT`, since a missing media drive must never take a service's own config down with it. `PITUNE_DATA_PATH` holds only `play_counts.json` (PiTune's own play-count tracker, see app/main.py); on an existing install upgrading to this, create it once by hand (`mkdir -p ./pitune/backend/data`) or re-run `setup.sh`, since the bind mount refuses to auto-create it. |
