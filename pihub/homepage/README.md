...
One container serves both the static frontend and the `/api/*` backend, so
no separate nginx is needed for this service, which is most of how it stays
...
model above) or because you're running the standalone compose file without
that line.
