...
problem comes from doing one while believing you are doing the other.
...
> another device is a **one-time file transfer**. There is no sync agent on your devices
> watching that folder. Nothing uploads your edits. Nothing pushes them anywhere.
>
> If you have uploaded your vault to the file browser and downloaded it on both devices, you
> have two independent copies that will drift apart from the moment you start editing. That is
> the expected behaviour of a file server, not a bug.
>
...
4. **There is no "create new vault / connect existing" choice.** That is Obsidian's paid Sync
   service, not this plugin. LiveSync syncs *the vault it is installed in* — one vault, one
   database. If you went looking for that option and could not find it, this is why.
...
**This is the one place FileBrowser is used, and it is a one-time transfer, not sync.**
...
`/mnt/data/files/` is FileBrowser's root — the `/srv` bind mount in `docker-compose.yml`. It
is **not** where LiveSync stores anything, and putting a vault there does not make it sync.
...
> Fauxton is on port **8443**, at the root. It does not work under `/couchdb/` — its bundle
...
1. **Test Database Connection** → must succeed. If it fails, jump to Troubleshooting **T4**.
2. **Check database configuration** → shows a list of CouchDB settings with ✅ or ⚠️. Our
...
You are telling the plugin: **local → remote**. The remote is empty; this device has the data.

- In the **setup wizard**, this is the answer to "is this the first device" / "does the remote
  already have data" — you are the first device, the remote has nothing.
- Done manually, it is the maintenance action that **sends this device's vault to the server**.
  Depending on plugin version it is labelled **"Rebuild everything"**, **"Overwrite remote"**,
...
> wipes your notes.
>
> Labels move between plugin versions, so do not trust the label alone. Read which side is the
...
Do not assume. Check the server:
...
uploaded and the push did not happen.
...
This is the single most important step. A second vault with existing content that has not been
...
produces a device that connects happily and then cannot decrypt anything.

1. **Desktop:** Settings → Self-hosted LiveSync → **Copy setup URI**. It asks for a one-time
   passphrase to encrypt the URI — invent one, you will type it once on the phone.
...
It has no client-side agent, it does not watch your vault folder, and it will never upload an
...
Do not skip 1.8 — verifying a non-zero document count is what distinguishes "sync configured"
from "sync working".
...
2. **Is sync actually switched on?** Settings → Self-hosted LiveSync → **Sync Settings**.
   ...
   are off, the plugin is installed and configured and will do nothing at all — a very common
...
4. **Is the server receiving anything?** Run `scripts/obsidian-check.sh` on the Pi, edit a note,
   run it again. If `doc_count` does not move, nothing is being sent and the problem is on that
...
histories. They usually say so in the Sync Status panel, but it is easy to miss, and from the
...
Editing `config/couchdb/zz-homedrive.ini` alone changes nothing until it is re-staged. See the
...
hand. If it has already gone wrong, fix the passphrase on the second device and
**"Fetch everything from the remote"** there.

### T7 — Both devices "work" but never see each other

**Cause:** they are pointed at different databases. Each syncs happily to its own.
...
If it lists `obsidian` **and** something like `obsidian2`, that is your answer. Fix the
database name in the plugin settings on the odd device out, then
**"Fetch everything from the remote"** there.

### T8 — Large files or attachments do not sync
...
(`/mnt/data/files/`) instead — that is what the file browser is good at.
...
> This is **not** real-time sync, and downloading the folder onto two devices does not make it
