...
Arbitrary environment variables — `${TS_HOSTNAME}`, `${TS_TAILNET}`, anything else — are
**not** expanded. They are passed to `tailscaled` verbatim, producing a serve config keyed
on a hostname that does not exist, and a stack that answers nothing on port 443 while every
...
API works fine under `/couchdb/` — that is the URI to give Obsidian LiveSync.
